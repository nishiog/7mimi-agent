...
```mermaid
flowchart LR
  subgraph host["ホスト (docker compose) — local/dev"]
...
本番(自宅 miniPC 上の k3s + ArgoCD、ADR-031/032/033)では、上記の compose sibling 構成は次の k8s 構成に置き換わる。ArgoCD が `deploy/k8s/` の Kustomize マニフェストを watch する GitOps 運用であり、docker.sock も internal ネットワークも存在しない。

```mermaid
flowchart LR
  subgraph argocd["ArgoCD"]
    APP["Application<br/>(deploy/k8s/ を watch)"]
  end
  subgraph k3s["k3s (namespace: 7mimi, RBAC限定)"]
    SCHP["scheduler Pod<br/>(Python, cron ループ)"]
    CPS["claude-proxy Service<br/>(Go) ANTHROPIC_API_KEY 保持"]
    APS["auth-proxy Service<br/>(Go) GitHub App key / X token / Slack bot token 保持"]
    EPS["egress-proxy Service<br/>(Go) CONNECT proxy"]
    JOB["runner Job / claude-digest Job<br/>(1リクエスト1Pod、credential ゼロ)"]
  end
  APP -- sync --> k3s
  SCHP -- "k8s API (Job create,<br/>namespace限定 RBAC)" --> JOB
  JOB -- "LLM (session token のみ)" --> CPS
  JOB -- "git push / MCP / Slack" --> APS
  JOB -- "WebFetch (HTTPS_PROXY)" --> EPS
  CPS --> ANT2["Anthropic API"]
  APS --> GH2["GitHub"] & X2["X API v2"] & SL2["Slack API"]
  EPS -- "公開 Web のみ" --> WEB2["一次情報サイト"]
  NP["NetworkPolicy<br/>default-deny + 3 proxy + kube-dns のみ許可<br/>(ADR-032)"]
```

### 4.1 ネットワークトポロジと認証情報の所在

compose(local/dev)は 2 つのネットワークを定義する。`7mimi-internal` は `internal: true` であり、**デフォルトルートを持たず外界へ出られない** ネットワークである。agent-runner はこのネットワーク **のみ** に接続される。3 つのプロキシは internal と通常の bridge(`egress`)の双方に接続点を持つため、runner から見た「外部への出口」はこの 3 経路に限定される。本番 k3s では同じ不変条件を NetworkPolicy(default-deny + 3 proxy + kube-dns のみ許可、ADR-032)が強制する。認証情報の所在と併せて図示すると次のとおりである(以下は compose/local-dev の図)。
...
- compose(local/dev)では、scheduler は docker.sock を保持し、定時に agent-runner を **sibling コンテナ** としてホストの Docker daemon 経由で起動する(compose 管理外のオンデマンド起動)。本番=k3s + ArgoCD(ADR-031)では docker.sock は存在せず、scheduler Pod が namespace 限定 RBAC の k8s API 経由で Job を起動する(`KubernetesRunnerBackend`)。
...
この構成は一度に到達したものではない。設計判断記録(ADR)は全 33 本存在し、初期はローカル実行の mock 収集(ADR-006)、次にホスト credential による暫定 publish(ADR-018)、git relay の完成に伴う当該暫定経路の廃止(ADR-020)と、段階的に「credential-free runner」へ収束させた。ADR-018 は当初から「自らを暫定と宣言する ADR」として記述し、後続の ADR が廃止を明記する運用を採っている。
...
もう一つの仕組みが **Stop hook による ADR 強制** である。アーキテクチャ・セキュリティ境界・config に触れる変更を行ったセッションは、`docs/planning/adr.md` に ADR を追記しない限り **完了がブロックされる**。「後でまとめて記述する」は発生し得ない。全 33 本の ADR がすべてその場で記述されたのは、規律ではなく hook の強制による。
...
|---|---|
| iptables DNAT でプロキシ強制 | Docker `internal: true` ネットワーク + 自前 egress-proxy(compose/local-dev)。本番: NetworkPolicy + egress-proxy(ADR-032) |
...
| GCE 上のセッション隔離コンテナ(copy-on-write) | セッションごとの workspace + 使い捨て sibling コンテナ(compose/local-dev)。本番: k8s Job(ADR-031) |
...
- **proxy ポートが LAN に開いている(compose/local-dev 限定)** — sibling runner が host-gateway 経由で到達する都合で 18080/18081 を全インターフェースへ bind しており、LAN 内の第三者に対してはセッション Bearer のみが防壁となる。信頼できないネットワークではホスト firewall で遮断する運用としている。k8s 本番は ClusterIP のみでポート公開はなく、この既知課題は compose/local-dev に限定される。
...
- 設計判断をその場で記録する(Stop hook が強制する全 33 本の ADR)
...
本システムのコードはすべて単一リポジトリに存在する。Go のプロキシ 3 つ、Python のオーケストレーション、hook、テスト、および設計判断の全履歴(ADR 全 33 本)である。追実装を試みる場合、ADR-001 から順に参照することで、mock のみの骨組みが credential-free の自律エージェントへ発展する過程を追跡できる。
