...
認可を hook で止めても、コンテナが外部と自由に通信できるなら、盗んだセッショントークンを外へ持ち出す(egress による持ち出し)余地が残る。メルカリはこれを **ネットワークレベル(iptables DNAT)** で解決し、宛先ごとに認証情報を自動注入していた。しかし macOS の Docker Desktop では iptables/DNAT を直接制御できない。そこで 7mimi-agent は、同じ狙いを **Docker ネイティブの internal ネットワーク + 単一の egress-proxy** に翻訳して実現する(ADR-025)。
...
この穴に対する答えが、実は第5章の egress-proxy(ADR-025)である。`Bash(git:*)` をコマンド面で完全には縛れない以上、**盗んだトークンを外へ持ち出す経路そのものを塞ぐ**ことで、多層防御の別の層で受け止める。ローカル dev(compose なし)の bridge 構成では egress が無制限のままであり、この守りは compose 環境で初めて効く点にも注意が必要である。「一つの層が不完全なら、別の層で補う」— 多層防御の本領がここに現れている。
...
- **docker.sock マウント**: scheduler は agent-runner を sibling コンテナとして起動するため `docker.sock` をマウントする。これは実質ホスト root 相当だが、scheduler イメージは自前コードのみで外部入力を実行しないため許容する、と判断されている(ADR-024)。
...
本書の締めくくりに、着想元であるメルカリ pcp-agent の構成と、7mimi-agent の実装を対応づける。同じ思想が、異なる制約(個人・macOS・Docker Desktop)の下で別の形に翻訳されていることが見てとれる。
...
| セッション隔離 | copy-on-write で独立コンテナ | 1ジョブ1コンテナの使い捨て(ADR-013、Phase 5 は deferred) |
| egress 強制 | iptables DNAT | internal 網 + 自前 egress-proxy(ADR-025) |
