...
部品が揃ったところで、「調査から公開まで」を 1 本に繋いだのが統合ジョブ **claude-digest** です(ADR-021、`runner/claude_digest.py`)。設計上の選択がいくつかあります。

**収集は orchestrator 側で事前に行う。** X API の呼び出しをコンテナ内 LLM にやらせず、orchestrator がクエリごとに PreToolUse 認可 → MCP `tools/call` → PostToolUse 監査、を回して結果を `signals.json` として workspace に置きます。X API 呼び出しを決定的な認可・監査の下に保ち、**コンテナ内 Claude の tool 面を最小化する**ためです。収集は per-query 耐障害で、個別クエリの失敗(レート制限など)はスキップして `failed_queries` に記録、全滅時のみ失敗。ただし**認可 deny だけは即時中断**します — 1 クエリへの deny は残り全クエリへの deny を意味する決定的判断だからです。

**allowedTools は最小 4 種。** コンテナ内 Claude は `Read,Write,WebFetch,Bash(git:*)` だけで起動します。signals.json を読み、トピックを選び、WebFetch で一次情報を確認し、日本語で書き、git relay 経由で push する — それ以外の道具はありません。
