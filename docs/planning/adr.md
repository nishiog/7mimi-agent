...
Reason: 対象ノードは containerd のみで Docker daemon が無く、docker.sock 経由の sibling 起動(ADR-024)は成立しない。k8s Job + namespace 限定 RBAC は docker.sock(実質ホスト root 相当)より blast radius が明確に小さく、セキュリティ改善でもある。config の ConfigMap 化は「Config is the source of truth」を GitOps に接続し、schedules/policy の変更をイメージ再ビルドなしで反映する。1ジョブ1Pod の使い捨て Job は ADR-013 の k8s 翻訳であり、Phase 5 常駐 runner(ADR-030 で deferred)を復活させるものではない。
